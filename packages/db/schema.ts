...
export const bookmarks = sqliteTable(
  "bookmarks",
  {
    ...
    summaryProvenance: text("summaryProvenance", {
      enum: ["web", "transcript", "manual"],
    }),
    summaryStale: integer("summaryStale", { mode: "boolean" })
      .notNull()
      .default(false),
...
export const bookmarkTranscripts = sqliteTable(
  "bookmarkTranscripts",
  {
    id: text("id")
      .notNull()
    ...
    sourceAttachmentsStatus: text("sourceAttachmentsStatus", {
    ...
    sourceIsAutoGenerated: integer("sourceIsAutoGenerated", {
      mode: "boolean",
    }),
    sourceTranscript: text("sourceTranscript"),
    text: text("text"),
    manualOverride: integer("manualOverride", { mode: "boolean" })
      .notNull()
      .default(false),
    revision: integer("revision").notNull().default(0),
    sourceFingerprint: text("sourceFingerprint"),
    sourceFetchedAt: integer("sourceFetchedAt", { mode: "timestamp" }),
    createdAt: createdAtField(),
    modifiedAt: modifiedAtField(),
  },
  (bt) => [
    unique("bookmarkTranscripts_bookmark_provider_unique").on(
      bt.bookmarkId,
      bt.provider,
    ),
    index("bookmarkTranscripts_bookmarkId_idx").on(bt.bookmarkId),
    index("bookmarkTranscripts_providerItemId_idx").on(bt.providerItemId),
  ],
);
...
export const assets = sqliteTable(
  "assets",
  {
    ...
    cleanupPending: integer("cleanupPending", { mode: "boolean" })
      .notNull()
      .default(false),
    transcriptId: text("transcriptId").references(
      () => bookmarkTranscripts.id,
      { onDelete: "cascade" },
    ),
    userId: text("userId")
      .notNull()
...
export const highlights = sqliteTable(
  "highlights",
  {
    ...
    transcriptRevision: integer("transcriptRevision"),
    color: text("color", {
...
export const userReadingProgress = sqliteTable(
  "userReadingProgress",
  {
    ...
    readingProgressRevision: integer("readingProgressRevision"),
    modifiedAt: modifiedAtField(),
  },
  (tb) => [
...
export const assetRelations = relations(assets, ({ one }) => ({
  ...
  transcript: one(bookmarkTranscripts, {
    fields: [assets.transcriptId],
    references: [bookmarkTranscripts.id],
  }),
}));

export const bookmarkTranscriptRelations = relations(
  bookmarkTranscripts,
  ({ one, many }) => ({
    bookmark: one(bookmarks, {
      fields: [bookmarkTranscripts.bookmarkId],
      references: [bookmarks.id],
    }),
    assets: many(assets),
  }),
