...
meili_port=$((7700 + WT_PORT_BASE))
chrome_port=$((9222 + WT_PORT_BASE))
tmp_env="$(mktemp)"
trap 'rm -f "$tmp_env"' EXIT

while IFS= read -r line || [[ -n "$line" ]]; do
  case "$line" in
    DATA_DIR=* | KARAKEEP_PORT=* | API_URL=* | NEXTAUTH_URL=* | MEILI_ADDR=* | BROWSER_WEB_URL=* | BROWSER_WEBSOCKET_URL=*)
      ;;
    *)
      printf '%s\n' "$line" >>"$tmp_env"
      ;;
...
MEILI_ADDR=http://localhost:$meili_port
...
echo "Configured worktree ports: web $web_port, Meilisearch $meili_port, Chrome $chrome_port"
