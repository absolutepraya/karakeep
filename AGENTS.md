...
- The workflow must not add timestamps or create an empty commit, and must fail without partial output when metadata, issue lookups, or either theme render is invalid. Pull request freshness checks compare the generated Excalidraw, all themed and compatibility SVG/PNG outputs, and README block, and verify that the rendered PNGs are non-empty and transparent at their corners; PNG bytes may vary across runner operating systems.
