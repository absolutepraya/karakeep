...
- Shared local infrastructure is machine-level: one Meilisearch at `http://localhost:7700` and one Chrome/CDP at `http://localhost:9223`. Manage it explicitly with `pnpm dev:infra:up`, `pnpm dev:infra:status`, and `pnpm dev:infra:down`.
...
- one machine-level Chrome container is shared at `http://localhost:9223`
...
- shared infra binds only to localhost; if ports `7700` or `9223` are occupied by something else, the helper fails rather than silently reusing an unknown service
