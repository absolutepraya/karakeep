...
- `marka-dev-chrome` on `127.0.0.1:9250`
...
BROWSER_WEB_URL=http://localhost:9250
```
...
- If port `7700` is occupied by something other than `marka-dev-meilisearch`, or port `9250` by something other than `marka-dev-chrome`, `pnpm dev:infra:up` fails instead of silently reusing an unknown service.
