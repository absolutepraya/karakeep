...
- **Media**: Images, PDFs, uploaded videos, or audio files you want to save for later. Images and PDFs are previewable in Karakeep, while MP4 and WebM videos support browser playback and download. MKV files are stored and downloadable, but browser playback is not guaranteed. Supported audio files use native browser playback with speed controls and a download fallback. Media files use the configured global upload-size limit, which defaults to 50 MB.
...
- **Uploaded files**: keep PDFs, videos, notes, or supporting assets right with the link.
