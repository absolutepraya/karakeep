...
- Asset bookmarks are migrated by downloading the original asset and re-uploading it to the destination. Images, PDFs, top-level video assets, and audio assets are supported. MP4 and WebM provide playback and download; MKV is preserved for download. Supported audio files use browser playback with a download fallback.
